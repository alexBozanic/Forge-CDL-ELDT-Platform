...
-- Final answer rows are server-grading internals. Aggregate attempt outcomes are
-- sufficient for school reporting; browser sessions must not read selections or
-- correctness flags (which could be combined to reconstruct an answer key).
drop policy if exists answers_read_admin on public.assessment_answers;
revoke select on public.assessment_answers from authenticated;
