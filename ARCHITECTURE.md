...
Assessment definitions, blueprints, prompts, options, and delivery metadata are part of the draft manifest and freeze at publication. Correct-option keys and each attempt's randomized payload live in the private schema. Starting an attempt pins the exact selected questions and option order; only database functions may grade it. Final assessment starts require every pinned manifest lesson, and integer cross-multiplication enforces the stored threshold without floating-point rounding. Submitted attempts and answers are append-only.

A passing final at or above 80 percent plus lesson prerequisites creates at most one immutable software completion per enrollment. The same transaction snapshots the course hash, reporting identity, provider identity, qualifying attempt, enrollment transition, and initial manual-reporting record. Missing required fields produce `needs_attention`, never invented values. Later preparation captures a new reporting snapshot without changing completion history. Submitted, accepted, and rejected are explicit append-only events; Forge does not call an FMCSA API.
